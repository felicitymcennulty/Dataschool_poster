...
```{r Figure1, echo=FALSE, out.width= "100%", fig.cap = "Spotted handfish, Brachionichthys hirsutus"}
...
TidyHistoric <- TidyHistoric %>% 
                  select(-On_transect) %>%
                  arrange(Transect_no) # this is only done to make the table more interesting in the poster

```

I also learnt to use regular expression (regex) coding in R to use str_replace to edit my location names between the 2 datasets where there were spelling differences in the data tables. See the example code below:
```{r Regex,echo=TRUE}
...
knitr::kable(head(TidyHistoric, n = 5), format = "html", caption = "A tidy data table") %>% 
...
**Tables**
```{r histable, out.width='100%', echo = T}
